  ...
  return <main>
    ...
    {screen === "home" && <section className="app-shell">
      <aside>
        ...
        <nav>{([["today","Today","⌁"],["routine","Routine builder","＋"],["catalogue","Catalogue","▦"],["progress","Stat tracker","↗"],["calendar","Calendar","□"]] as [Tab,string,string][]).map(([id,label,icon]) => <button key={id} onClick={() => setTab(id)} className={tab === id ? "active" : ""}><span>{icon}</span>{label}</button>)}</nav>
      ...
      <div className="workspace">
        ...
        {tab === "today" && <Today exercises={exercises} update={update} momentum={momentum} setTab={setTab} />}
        ...
        {tab === "catalogue" && <Catalogue addExercise={addFromCatalogue} restDays={restDays} />}
...
function Today({ exercises, update, momentum, setTab }: { exercises: Exercise[]; update:(id:number,p:Partial<Exercise>)=>void; momentum:number; setTab:(x:Tab)=>void }) {
...
function Catalogue({addExercise,restDays}:{addExercise:(item:CatalogueItem,day:string)=>void;restDays:Record<string,boolean>}){
...
function Routine({ exercises, update, remove, add, targets, notify, statuses, setStatuses, restDays, setRestDays }: { exercises:Exercise[]; update:(id:number,p:Partial<Exercise>)=>void; remove:(id:number)=>void; add:()=>number; targets:string[]; notify:(x:string)=>void;statuses:Record<number,string>;setStatuses:React.Dispatch<React.SetStateAction<Record<number,string>>>;restDays:Record<string,boolean>;setRestDays:React.Dispatch<React.SetStateAction<Record<string,boolean>>> }) {
  ...
  const addToDay=(day:string)=>{if(restDays[day]){notify(`${columnInfo.find(d=>d.id===day)?.label||day} is set to rest — no exercises can be added there.`);return;}const id=add();setStatuses(x=>({...x,[id]:day}));notify(`Exercise added to ${columnInfo.find(d=>d.id===day)?.label||day}.`)};
