...
function Routine({ exercises, update, remove, add, targets, notify }: { exercises:Exercise[]; update:(id:number,p:Partial<Exercise>)=>void; remove:(id:number)=>void; add:()=>number; targets:string[]; notify:(x:string)=>void }) {
  ...
  return <div className="builder board-builder fade-up">
    ...
    <div className={`kanban weekly-kanban ${dayFilter!=="all"?"single-day":""}`}><section className="kanban-column ai-column"><header><div><i/><b>Built with AI</b><span>{aiSuggestions.length}</span></div><span>DRAG →</span></header><div className="kanban-cards">{aiSuggestions.map(e=><article className="workout-card ai-suggestion" draggable onDragStart={event=>startDrag(event,"ai",e.id)} key={e.id}><div className="card-status"><label><span>✦ AI CURATED</span></label><b className={e.difficulty>7?"high":"medium"}>{e.difficulty>7?"HIGH":"MEDIUM"}</b></div><strong className="ai-card-name">{e.name}</strong><p>{e.sets} sets × {e.reps} reps targeting {e.muscle.toLowerCase()}.</p><ExerciseMuscleMap name={e.name} muscle={e.muscle}/><small className="drag-hint">⠿ Drag into a training day</small></article>)}</div>{!aiSuggestions.length&&<div className="ai-empty"><b>✦</b><span>Describe a goal above to generate curated workouts.</span></div>}</section>{visibleColumns.map(col=>{const cards=exercises.filter(e=>(statuses[e.id]||"monday")===col.id&&passesFilters(e));return <section className="kanban-column day-dropzone" onDragOver={e=>e.preventDefault()} onDrop={e=>dropOnDay(e,col.id)} key={col.id}>
      <header><div><i className={col.dot}/><b>{col.label}</b><span>{cards.length}</span></div><button onClick={()=>addToDay(col.id)}>＋</button></header>
      <div className="kanban-cards">{cards.map((e,i)=><article className={`workout-card ${e.done?"done":""}`} draggable onDragStart={event=>startDrag(event,"scheduled",e.id)} key={e.id}>
